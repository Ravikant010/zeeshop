...
export  function Item({ 
    ...
    const [ImageUrlCounter, setImageCounter] = useState<number>(0)
    const { toast } = useToast();
    const route = usePathname();

    useEffect(()=>{
        if(isHovered)
        setImageCounter(ImageUrlCounter+1)
    else
    setImageCounter(0)
    }, [isHovered])
    ...
    return (
        ...
        >
            ...
            <div className="relative w-full pt-[100%]">
                <Image 
                    src={product.image_urls[ImageUrlCounter]} 
                    alt={product.product}
                    layout="fill"
                    objectFit="cover"
            ...
            {isHovered && (
                 <Button 
                 className="absolute bottom-5 left-1/2 -translate-x-1/2 w-10/12 p-2 py-0 rounded-full text-center bg-[#a3b18a] border border-black hover:bg-white hover:text-black capitalize font-sans font-normal"
